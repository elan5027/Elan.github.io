...
젠킨스 메인화면에서 새로운 Item을 클릭한다.

Freestyle project를 선택 후 이름을 편한대로 작성한다.

소스 코드 관리에서 Git을 선택하고 Repository URL에 이전에 연력하려 작업한 깃허브 레포지토리의 SSH 주소를 넣어준다.

Credentials 옵션은 이전에 등록해둔 키를 선택한다.
Branch Specifier 옵션은 Push를 탐지할 Branch의 이름을 넣어준다.
Main 브랜치에 대한 탐색을 한다면 \*/main을 작성한다.

빌드 유발 탭 -> GitHub hook trigger for GITScmpolling 체크

Build 탭 -> Excute shell 탭 선택 -> 빌드와 테스트를 위한 코드를 작성후 완료한다.
...
젠킨스에 exec를 통해 쉘로 접속한 후

/var/jenkins_home/workspace/jenkins-ci-cd/ 로 이동한다.

이곳에 기존 프로젝트의 환경 설정 파일을 작성한다.
빌드를 위한 Dockerfile을 작성한다.
...
/var/jenkins_home/CICD/ 위치에
Docker_CD.sh 이름으로 작성하였다. CICD는 존재하지 않는 디렉토리를 만든것이다.
...
젠킨스 설정으로 이동 한 후 하단에 Add build step을 클릭 후
Execute shell을 추가하여 방금 작성한 쉘파일을 실행시킨다.
...
운영을 위한 서버를 생성한다.
젠킨스 및 SSL 접속을 위한 22번 포트와
443 HTTPS 및 80포트를 열어준다.

젠킨스 웹페이지로 이동한 후 젠킨스 관리 -> Plugins 로 이동한다.
사이드 바에 Available plugins를 누른다.
Publish Over SSH와 SSH plugin을 설치한다.
[ Publish Over SSH만 설치하면 되는듯 하다 ]
[ 원래는 재시작을 해야 될지 모르나 그냥 메인페이지로 이동하고 해도 동작햇다. ]

메인페이지로 이동 한 후 젠킨스 관리 -> System으로 이동

Publish over SSH 탭을 찾을때까지 휠을 내려준다.
...
설정파일을 관리할 deploy 폴더를 만든다

mkdir deploy
cd deploy

우선 이미지를 올리기 위해 사용될 docker-compose.yml 파일을 작성한다.
기본적인 세팅은 아래와 같다.
...
작성 이후 젠킨스 웹홈페이지로 이동하여

만든 작업의 빌드 편집 환경으로 이동한다.

빌드 후 조치 탭 -> Send build artifacts over SSH 클릭
